...
These implementations are called with function `condKendall`, whose arguments are as follows:
```{R}
args(condKendall)
```
...
If this vector is not specified, `condKendall` assumes all observed failure times are events.
...
The corresponding quasi-independence test can be called with the function `condKendall`
```{R}
condKendall(dat$T, dat$Y, dat$delta)
```
...
```{R}
condKendall(dat$T, dat$Y, dat$delta)$PE
condKendall(dat$T, dat$Y, dat$delta)$p.value
```
...
When `method = "IPW1"`, `condKendall` compute the conditional Kendall's tau defined as $\widehat\tau_{c2}$ 
in Section 4 of @austin2014eliminating.

```{R}
condKendall(dat$T, dat$Y, dat$delta, method = "IPW1")
```

#### @austin2014eliminating

When `method = "IPW2"`, `condKendall` compute the conditional Kendall's tau defined as $\widehat\tau_{c3}$
in Section 4 of @austin2014eliminating.

```{R}
condKendall(dat$T, dat$Y, dat$delta, method = "IPW2")
```

## Channing House study
We illustrate the use of `condKendall` on the Channing House data set, 
